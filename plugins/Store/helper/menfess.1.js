exports.run = {
   usage: ['menfess'],
   hidden: ['delses', 'again', 'selesai', 'confes', 'confess'],
   category: 'utilities',
   async: async (m, {
      client,
      text,
      ...
      try {
         ...
         if (['confess', 'confes'].includes(command)) {
            ...
            if (isReceiverOn) return client.reply(m.chat, Func.texted('bold', `🚩 Saat admin sedang melakukan percakapan dengan orang lain.`), m)
            client.reply(m.chat, Func.texted('bold', `✅  Kamu sudah terhubung* \nSilahkan mulai mengirim pesan/gambar!\n\nJika sudah selesai silakan balas *.delses*\nGunakan titik di awal delses = .delses\n\nTerima kasih`), m)
            ...
            msg += `➠ *${isPrefix}selesai ~ menutup percakapan\n`
